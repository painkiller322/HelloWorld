...
public class PredicateExample {
	static List<HighStudent> list = Arrays.asList(new HighStudent("홍길동", "남자", 80), new HighStudent("김순희", "여자", 90),
			new HighStudent("박길동", "남자", 85), new HighStudent("박순희", "여자", 95));

	static double avg(Predicate<HighStudent> pred) {
		boolean bool = false;
		int cnt = 0, sum = 0;
		for (HighStudent student : list) {
			bool = pred.test(student);// getSex()="남자"값만
			if (bool) {
				cnt++;
